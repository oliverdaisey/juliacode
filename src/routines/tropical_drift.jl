...
function tropical_drift(T::MixedCellTracker)
    ...
    lengths = [length(x.dualWeight) for x in T.mixed_cell.dual_cells]
    ...
    for i in 1:length(new_mixed_cell.dual_cells)
        new_mixed_cell.dual_cells[i].dualWeight = new_mixed_cell.dual_cells[i].dualWeight .* TT.(epsilon * [x.data for x in dual_direction_vectors[i]])
    end
